...
export default function RemiHomeScreen() {
  const { t, i18n } = useTranslation('remi');
  const [games, setGames] = useState<RemiGame[]>([]);
  ...
  const handleDelete = async (g: RemiGame): Promise<void> => {
      ...
      {games.length === 0 ? (
        ...
        <ul className="space-y-2">
          {games.map((g) => {
            ...
            const modeLine =
              g.endCondition.kind === 'targetScore'
                ? t('game.targetMode', { target: g.endCondition.targetScore })
                : t('game.handCountMode', {
                    n: Math.min(g.hands.length + 1, g.endCondition.handCount),
                    total: g.endCondition.handCount,
                  });
            return (
              <li
                key={g.id}
              ...
              >
                <Link to={`/remi/game/${g.id}`} className="flex-1">
                  ...
                  <div className="text-xs text-slate-400">
                    {t('home.createdAt')}:{' '}
                    {new Date(g.createdAt).toLocaleString(i18n.resolvedLanguage)} · {modeLine}
                    {leader && (
                      <>
                        {' · '}
