...
export default function WhistHomeScreen() {
  const { t, i18n } = useTranslation('whist');
  const [games, setGames] = useState<WhistGame[]>([]);
  ...
  const handleDelete = async (g: WhistGame): Promise<void> => {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try {
      // Auto-detects scoring-pad, whist-scoring-app, or rentz-scoring-app —
        ...
        <ul className="space-y-2">
          {games.map((g) => {
            ...
            return (
              ...
              >
                <Link to={`/whist/game/${g.id}`} className="flex-1">
                  ...
                  <div className="text-xs text-slate-400">
                    ...
                    {g.rounds.length}/{g.schedule.length}
                    {leader && (
                      <>
                        {' · '}
